...
export const data = new SlashCommandBuilder()
  .setName('gemma4')
  .setDescription('Gemma に質問する・軽量版 e2b（単発・履歴なし）')
...
export async function execute(interaction: ChatInputCommandInteraction): Promise<void> {
  ...
  // Ollama の応答は3秒以内に返らないため、先に deferReply() で Discord に通知する
  await interaction.deferReply();

  try {
    const responseText = await generateResponse(prompt, 'gemma4:e2b');
